    ...
    while(True):
        try:
            ...
            for row in result['data']:
                ...
                changePercent = float(row['attributes']['price_percent_changes']['last_1h'][:-1])
                name = row['attributes']['name']
                address = row['attributes']['address']
                vol = float(row['attributes']['to_volume_in_usd'])
                if vol > 10000 and liq > 50000 and float(changePercent) > 5 and 'WBNB' in name:
    ...
    while True:
        try:
            ...
            for row in result['data']:
                ...
                if not timeIsForLastHourAgo(time, 7200):
                    findLastDate = True
                    break
            pageNumber = pageNumber + 1
        except Exception as ex:
...
for coin in coins:
    ...
    send_msg(coin[2]+ " " + coin[1] + " addresses: " + str(addresses))
