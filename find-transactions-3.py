...
open('scams-transactions', 'a+')
data = open('scams-transactions', 'r').read()
if data == '':
    data = '{}'
scamsTransactions = json.loads(data)
addresses = open('scams', 'r').read().split('\n')
...
def waitForScamsComing(address):
    ...
    for row in rows:
        ...
        if address not in scamsTransactions.keys():
            scamsTransactions[address] = {}

        if 'lastTx' in scamsTransactions[address].keys():
            if scamsTransactions[address]['lastToken'] != data['token'] and ('sent' not in scamsTransactions[address].keys() or scamsTransactions[address]['sent'] != data['tx']):
                if data['value'] != '1':
                    if check_time_difference(data['time']):
                        scamsTransactions[address]['sent'] = data['tx']
                        send_msg(pretty(data))

        scamsTransactions[address]['lastTx'] = data['tx']
        scamsTransactions[address]['lastToken'] = data['token']

for address in addresses:
    waitForScamsComing(address)

json.dump(scamsTransactions, open('scams-transactions', 'w+'))
